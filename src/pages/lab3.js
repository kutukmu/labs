...
import { IoMdSync, IoIosCloseCircleOutline, IoIosArrowRoundForward } from "react-icons/io"
const Lab3 = () => {
    ...
    state.arr.forEach((n,id)=>{
        ...
        n.forEach((m,idx)=>{
            ...
            if(m > 0 && m < 1000){
                ...
                window["edge"+firstName+secondName] = new GraphEdge(vertexArr[id], vertexArr[idx], Number(m) );
     ...
     vertexArr.forEach((n,id)=>{
             ...
             const { distances:distances1, previousVertices:previousVertices1, list:list1 } = Bellman(graph, vertexArr[id]);
            listarr.push(list1)
        ...
        <Layout>
            <div className="lab3">
                ...
                <div className="lab3-inner">
                    ...
                    {!hide &&  <form className="select-form" onSubmit={handleSubmit} >

                        <label htmlFor="numnode">Please Select the Number of Nodes </label>
                            ...
                            <button type="submit" className="btn blue">Continue <IoIosArrowRoundForward className="icon"/></button>
                            </div>
                    ...
                    {isSelected && <h3>Please Enter The Node Names</h3>}
                     {isSelected &&   <form onSubmit={handleNameSubmit}>
                        {nodeName.namearr.map((n,idx)=>{
                            return <input name={`${idx}`} onChange={handleNameChange} value={n}  type="text" />
                        })}
                        <div className="buttons">
                           <button type="submit" className="btn blue">Continue <IoIosArrowRoundForward className="icon"/></button>
                        </div>
                    ...
                    {isEntered && <div className="input-box">
                                    <h3>Please enter the Original Cost</h3>
                                        <table>
                                            ...
                                            {name.tablearr.slice(1).map((n, id) => {
                                                return <td key={id} ><input type="text" name={`${idx}-${id}`} value={state.arr[idx][id]} onChange={handleChange}></input></td>
                                            })}
